...
const ButtonCopy: React.FC<CopyButtonProps> = ({ text }) => {
  ...
  return (
    <button
      onClick={handleCopy}
      className="group relative inline-flex items-center rounded-full bg-black pb-1 text-sm font-medium text-black shadow-sm transition-all duration-200 ease-in-out hover:outline-none hover:ring-2 hover:ring-offset-2">
      {copied ? (
